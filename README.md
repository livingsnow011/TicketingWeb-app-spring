...
## 후기 
### aws 서비스들을 정리하며...
이 프로젝트는 배포 과정이 끝나는 5월부터 시작해 수료 후, **내가 처음부터 다 만들 수 있는 내 프로젝트가 맞나** 생각하여 이력서를 넣는 중간에도 틈틈히 처음부터 다시 만들어본 프로젝트입니다.   

그만큼 애착이 생겨버렸고 처음에는 다른 팀원의 아이디어로 시작한 프로젝트였지만, 이미지를 포함한 게시글과 회원서비스를 통한 추첨예매가 언젠가는 **진짜 이런 서비스가 나올 수도 있겠구나** 생각하게 만들었습니다.  

aws서비스를 정리하면서 아쉬움이 많이 남지만 가장 후회되는 것은, 작년 클라우드에 대한 공부를 시작했을 때 배포할 간단한 웹 애플리케이션도 없이 무작정 aws 서비스들을 찾아보고 정리하는 식으로 공부했던 것이 제일 후회가 되는 점입니다.  

**제가 참여한 프로젝트를 통해 클라우드 서비스들을 사용할 때 가장 큰 경험이 된 것 같습니다.**    

ec2, rds, s3, IAM, codedeploy 등이 제가 직접 작성한 프로젝트에 적용할 때 이게 이렇게 쓰이는 구나를 체감할 수 있었습니다.  

또한 교육과정 시작 전 제가 그렇게 얻고 싶었던 협업을 통한 개발도 할 수 있어서 제겐 더 뜻 깊은 프로젝트였습니다.  

취준 기간 동안 다음 프로젝트는 견고한 게시판 프로젝트를 생각하고 있습니다.   

이번 프로젝트보다는 약간 심심한 주제이지만, 최소한의 구현력으로 만든 이번 프로젝트와는 달리 좀 더 스프링과 db, 웹에 대해 더 깊숙한 주제로 만들어 보고 싶습니다. 
